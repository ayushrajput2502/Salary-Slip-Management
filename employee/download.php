...
if (isset($_POST['down-sal-button'])) {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        // $pdf->Image('../image/apsit.png',40,69,130);
...
}elseif(isset($_POST['current_print'])){
    $id = $_SESSION['id'];
    $date = $_POST['current_date'];
    $pdf->AddPage();
    $query = "SELECT * FROM salary as s INNER JOIN employee_details as e ON s.emp_id=e.emp_id WHERE s.emp_id = '$id' AND DATE_FORMAT(date, '%Y-%m')='$date'";
        $result = mysqli_query($conn, $query);
        $row = $result->fetch_assoc();
